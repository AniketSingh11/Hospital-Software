...
class Investigation extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model(array(
            'hospital_activities/investigation_model',
            'doctor_model'
        ));
    ...
    public function index()
    {
        ...
        $data['content'] = $this->load->view('hospital_activities/investigation_view',$data,true);
    ...
    { 
        ...
        if (empty($id)) {
            ...
            if ($this->form_validation->run() === true) { 
                ...
                redirect('hospital_activities/investigation/form');
            } else {
                $data['title'] = display('add_investigation_report');
                $data['doctor_list'] = $this->doctor_model->doctor_list();
                $data['content'] = $this->load->view('hospital_activities/investigation_form',$data,true);
                $this->load->view('layout/main_wrapper',$data);
            } 
            ...
            if ($this->form_validation->run() === true) { 
                ...
                redirect('hospital_activities/investigation/form/'.$postData['id']);
            } else {
    ...
    public function delete($id = null) 
    {
        ...
        redirect('hospital_activities/investigation');
    }
